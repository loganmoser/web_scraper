...
|---|---|
| `heading` | first h1 tag, falling back to the first h2 tag |
| `first_paragraph` | first paragraph tag inside the main tag, falling back to the first paragraph tag anywhere |
| `outgoing_links` | every `href` attribute on the page, turned into an absolute URL |
| `image_urls` | every `src` attribute on the page, turned into an absolute URL |
